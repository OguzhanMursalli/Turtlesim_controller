...
class turtle_pid(Node):
    def __init__(self):
        ...
        self.pid_len = PID(0.5,0.08,0.01,setpoint = 180)
    ...
    def turtle2_callback(self,pose:Pose):
        ...
        if hipo > 0.01:
            linear_ = self.pid_len(hipo)
        else:
            self.kill_turtle("turtle2")
            linear_ = 0.0
            angular_ = 0.0
        self.tw_loop(angular_,linear_)

    def tw_loop(self,angular_,linear_):
        msg_ = Twist()
        msg_.angular.z = angular_
        msg_.linear.x = linear_
